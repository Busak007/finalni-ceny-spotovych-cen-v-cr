...
class SpotElectricityConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    def async_get_options_flow(config_entry):
        ...
        return SpotElectricityOptionsFlow(config_entry)


class SpotElectricityOptionsFlow(OptionsFlow):
    """Handle options flow for Spot Electricity Price integration."""
    ...
    async def async_step_init(self, user_input=None):
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema({
                vol.Required(CONF_DISTRIBUTOR, default=get(CONF_DISTRIBUTOR)): vol.In(REGION),
                vol.Required(CONF_A, default=get(CONF_A)): str,
                vol.Required(CONF_PRICE_NT, default=get(CONF_PRICE_NT, 0.22264)): float,
                vol.Required(CONF_PRICE_VT, default=get(CONF_PRICE_VT, 2.12308)): float,
                vol.Required(CONF_SPOT_PRICE_ENTITY, default=get(CONF_SPOT_PRICE_ENTITY)): str,
                vol.Required(CONF_HDO_ENTITY, default=get(CONF_HDO_ENTITY)): str,
                vol.Required(CONF_PRODEJ, default=get(CONF_PRODEJ, 0.35)): float,
                vol.Required(CONF_DAN_Z_ELEKTRINY, default=get(CONF_DAN_Z_ELEKTRINY, 0.0283)): float,
                vol.Required(CONF_CENA_SYSTEMOVYCH_SLUZEB, default=get(CONF_CENA_SYSTEMOVYCH_SLUZEB, 0.17092)): float,
                vol.Required(CONF_OZE, default=get(CONF_OZE, 0.495)): float,
                vol.Required(CONF_VYKUP, default=get(CONF_VYKUP, -0.45)): float,
            })
        )
